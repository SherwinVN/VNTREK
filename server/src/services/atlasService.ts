...
function loadGeoBundle(name: 'admin0' | 'admin1' | 'vn_provinces' | 'vn_boundary'): any {
...
/** Lightweight VNM boundary with islands merged — single feature for overlay on non-atlas maps. */
export function getVnBoundaryGeo(): any {
  return loadGeoBundle('vn_boundary');
}
...
export async function reverseGeocodeCountry(lat: number, lng: number): Promise<string | null> {
  ...
  try {
    const res = await fetch(`https://nominatim.openstreetmap.org/reverse?lat=${lat}&lon=${lng}&format=json&zoom=3&accept-language=en`, {
      headers: { 'User-Agent': 'VNTREK Travel Planner (https://github.com/SherwinVN/VNTREK)' },
